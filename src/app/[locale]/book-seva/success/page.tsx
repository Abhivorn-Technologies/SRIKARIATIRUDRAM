...
import { CheckCircle2, Download, Home, Printer, Share2, Sparkles, Star, ArrowLeft } from 'lucide-react';
...
export default function BookingSuccessPage() {
  ...
  useEffect(() => {
    async function loadBooking() {
      if (bookingIdParam) {
        const found = await bookingService.getBookingById(bookingIdParam);
        if (found) {
          setBooking(found);
          return;
        }
      }
      const all = await bookingService.getDevoteeBookings();
      if (all && all.length > 0) {
        setBooking(all[0]);
      }
    }
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 print:p-0 print:max-w-none">
        ...
        <div className="flex items-center justify-between print:hidden">
          ...
          >
            ...
            <span>{isTe ? 'ప్రధాన పేజీ' : isHi ? 'मुख्य पृष्ठ' : 'Home'}</span>
          </Link>
        </div>

        {/* Progress Stepper: Step 5 CONFIRMATION (hidden during print) */}
        ...
        >
          ...
          <div className="p-6 rounded-2xl bg-[#35030A]/90 border border-[#D6A532]/40 text-left space-y-4 font-sans print:bg-gray-50 print:border-gray-300 print:text-black">
            <div className="flex items-center justify-between border-b border-[#D6A532]/25 pb-3">
              ...
              <span className="px-3 py-1 rounded-full bg-emerald-950 border border-emerald-500/50 text-emerald-400 font-cinzel text-xs font-black tracking-wider uppercase">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs sm:text-sm">
              ...
              <div className="space-y-0.5">
                ...
                <span className="font-bold text-[#FAF4E6] print:text-black">
                  {displayDate} {displayDayNum ? `(${displayDayNum})` : ''}
                </span>
              </div>

              <div className="space-y-0.5">
                <span className="text-[10px] uppercase font-bold text-[#E8C76A]/80 tracking-widest block">
                  {isTe ? 'జన్మ నక్షత్రం' : 'Janma Nakshatra'}
                </span>
                <span className="font-bold text-[#FAF4E6] print:text-black">
                  {displayNakshatra}
                </span>
              ...
              <div className="space-y-0.5">
                <span className="text-[10px] uppercase font-bold text-[#E8C76A]/80 tracking-widest block">
                  {isTe ? 'సమర్పించిన విరాళం' : 'Donation Amount'}
                </span>
              ...
              <div className="space-y-0.5 sm:col-span-2 pt-1 border-t border-[#D6A532]/20">
                <span className="text-[10px] uppercase font-bold text-[#E8C76A]/80 tracking-widest block">
                ...
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider font-cinzel ${
                  (booking?.devoteeParticipation === 'attending' || booking?.primaryDevotee.attendingPersonally === 'yes' || booking?.primaryDevotee.devoteeParticipation === 'attending')
                  ...
                  <span>{(booking?.devoteeParticipation === 'attending' || booking?.primaryDevotee.attendingPersonally === 'yes' || booking?.primaryDevotee.devoteeParticipation === 'attending') ? '🪔' : '📦'}</span>
                  <span>
                    {(booking?.devoteeParticipation === 'attending' || booking?.primaryDevotee.attendingPersonally === 'yes' || booking?.primaryDevotee.devoteeParticipation === 'attending')
                      ? 'ATTENDING IN PERSON'
