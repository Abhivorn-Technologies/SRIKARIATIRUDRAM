...
export async function PATCH(
  ...
  try {
    ...
    if (!updated) {
      return NextResponse.json({ success: false, error: "Seva update failed" }, { status: 404 });
    }
...
export async function DELETE(
  ...
  try {
    const success = await sevaServerService.deleteSeva(params.id);
    if (!success) {
      return NextResponse.json({ success: false, error: "Seva deletion failed" }, { status: 404 });
    }
